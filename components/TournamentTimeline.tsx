    ...
    <section className="py-24 bg-white">
      <div className="max-w-4xl mx-auto px-6">
        <div className="text-center mb-20">
          <span className="section-label mb-6">INTENSITY</span>
          <h2 className="font-display text-7xl md:text-8xl uppercase tracking-normal" style={{ lineHeight: '0.9' }}>
        ...
        <div className="space-y-1">
          {events.map((e, i) => (
            <div key={i} className="flex group border-b border-zinc-100 last:border-0 hover:bg-stadium-gray transition-colors">
              <div className="w-32 py-8 font-display text-4xl text-cesar-gold border-r border-zinc-100 flex items-center justify-center bg-black group-hover:bg-cesar-gold group-hover:text-black transition-colors shrink-0">
                {e.time}
              </div>
              <div className="p-8">
                <h4 className="font-display text-3xl uppercase mb-1 tracking-tight">{e.event}</h4>
                <p className="text-xs text-zinc-400 font-bold uppercase tracking-widest">{e.desc}</p>
              </div>
            </div>
          ))}
        </div>
